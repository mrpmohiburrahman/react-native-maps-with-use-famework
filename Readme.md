...
4. Error:
    >  Cycle inside FBReactNativeSpec; building could produce unreliable results. This usually can be resolved by moving the shell script phase '[CP-User] Generate Specs' so that it runs before the build phase that depends on its outputs. Cycle details: → Target 'FBReactNativeSpec': Libtool /Users/ghost/Library/Developer/Xcode/DerivedData/reactNativeMapsIOSv3-fwhpgvkrroaqoxcossewmfjhfgnq/Build/Products/Debug-iphonesimulator/FBReactNativeSpec/FBReactNativeSpec.framework/FBReactNativeSpec normal ○ Target 'FBReactNativeSpec' has compile command with input '/Users/ghost/Documents/ReactNative/reactNativeMapsIOSv3/ios/Pods/Target Support Files/FBReactNativeSpec/FBReactNativeSpec-dummy.m' ○ That command depends on command in Target 'FBReactNativeSpec': script phase “[CP-User] Generate Specs” ○ Target 'FBReactNativeSpec' has copy command from '/Users/ghost/Documents/ReactNative/reactNativeMapsIOSv3/node_modules/react-native/React/FBReactNativeSpec/FBReactNativeSpec/FBReactNativeSpec.h' to '/Users/ghost/Library/Developer/Xcode/DerivedData/reactNativeMapsIOSv3-fwhpgvkrroaqoxcossewmfjhfgnq/Build/Products/Debug-iphonesimulator/FBReactNativeSpec/FBReactNativeSpec.framework/Headers/FBReactNativeSpec.h' ○ That command depends on command in Target 'FBReactNativeSpec': script phase “[CP-User] Generate Specs”```
...
1. Add AirMaps and AirGoogleMaps. [source](https://github.com/react-native-maps/react-native-maps/issues/3536#issuecomment-683563649)
    ...
    1. Make sure the option ```create groups``` and your target is selected.
1. Add google-maps-ios-utils: According to these official [instruction](https://github.com/googlemaps/google-maps-ios-utils/blob/b721e95a500d0c9a4fd93738e83fc86c2a57ac89/Swift.md#integrating-with-swift-projects-which-use-use_frameworks-in-the-podfile)
    1. ```git clone https://github.com/googlemaps/google-maps-ios-utils master``` on ios folder with a different name. We need only the content of ```src``` folder. You can delete other files and folders
    1. On XCODE, Right click on a project and create a group named ```Google-Maps-iOS-Utils```.
...
1. Create Swift Bridging Header
    ...
    1. Usually the name would be [Project Name]-Bridging-Header.h but you name it whatever you want. I am naming it just ```Header.h``` in this project as it is really a simple project.
...
1. Change to import statement to ```#import "GMUHeatmapTileLayer.h"``` in ```AirGoogleMaps/AIRGoogleMapHeatmap.h``` file.
